...
import { Data, clearCart, minusItem, plusItem, removeItem } from "../redux/slices/cartSlice";
...
const Cart = () => {
    ...
    const onClickClearCart = () => {
        if(window.confirm("Очистити кошик?")) dispatch(clearCart())
    }
      React.useEffect(() => {
  ...
  return (
    <div>
    {isModalShown && <div className="modal__wrapper" ref={ref}><ModalWindow totalPrice = {totalPrice}/></div>} 
      <div className="container">
        {item.length>0 ? <>   <img onClick={() => onClickClearCart()} className="trashCart" src="/img/trash.svg" alt="trash" />      <div className="header">
          <div>Товар</div>
          <div>Назва</div>
          <div>Кількість</div>
          <div>Ціна</div>
      ...
      <h3>Всього: {totalCount} товарів на {totalPrice}₴</h3>
      <div ref={ref2} onClick={() => setIsModalShown(!isModalShown)} className="orderButton">Замовити</div></div></> : <h2>Кошик порожній.</h2>}
      </div>
    </div>
  );
};
